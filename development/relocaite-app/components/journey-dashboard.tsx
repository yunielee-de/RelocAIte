...
export function JourneyDashboard({ profile, assessment, onNavigate }: { profile: JourneyProfile; assessment: VisaAssessment; onNavigate: (view: JourneyView) => void }) {
  ...
  return <div className="reference-dashboard">
    ...
    <div className="journey-stage-list">{journeyStages.map((stage, index) => {
      ...
      const destination: JourneyView = stage.id === "contract" ? "contract-overview" : stage.id === "visa" ? "visa-overview" : stage.id;
      return <button key={stage.id} type="button" className={`panel journey-stage-row ${active ? "current" : ""}`} aria-label={`Open step ${index + 1}: ${stage.title}`} onClick={() => onNavigate(destination)}>
        <span className="stage-number">{index + 1}</span><span className={`stage-icon ${complete ? "complete" : ""}`}><Icon size={26} /></span>
        <div className="stage-copy"><h3>{stage.title}</h3><p>{stage.description}</p><span className={`stage-link ${active ? "primary" : ""}`}>{active ? "Continue" : "View details"}{active && <ArrowRight size={15} />}</span></div>

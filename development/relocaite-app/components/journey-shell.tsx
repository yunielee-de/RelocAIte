...
import { ArrowRight, BookOpen, FileText, House, Map, Menu, MessageCircle, RotateCcw, ShieldCheck, X } from "lucide-react";
import { useRef, useState, type ReactNode } from "react";
import { Button } from "@/components/ui/button";
import type { StageId } from "@/lib/journey-profile";

export type JourneyView = "home" | "journey" | "documents" | "assistant" | "resources" | StageId;
const navigation = [
...
export function JourneyShell({ view, onNavigate, name, saved, onReset, children }: {
  ...
  return <div className="journey-app">
    ...
    <div className="mobile-bar">
      <button onClick={() => navigate("home")} aria-label="RelocAIte home"><Image src="/relocaite-logo.jpg" alt="RelocAIte" width={180} height={41} priority unoptimized /></button>
    ...
    <aside className={`journey-sidebar ${menuOpen ? "is-open" : ""}`}>
      <button className="sidebar-brand" onClick={() => navigate("home")} aria-label="RelocAIte home"><Image src="/relocaite-logo.jpg" alt="RelocAIte" width={180} height={41} priority unoptimized /></button>
      <p className="sidebar-caption">YOUR NEW CHAPTER</p>
      <nav aria-label="Main navigation" className="sidebar-nav">{navigation.map(item => {
        const active = view === item.id || item.id === "journey" && ["contract", "visa", "accommodation", "setup", "benefits"].includes(view);
    ...
    <div className="journey-workspace">
      <header className="workspace-topbar"><span className="topbar-label">Your move to Germany <ArrowRight size={14} /></span><div className="topbar-user"><span className="save-indicator" role="status"><span />{saved}</span><span className="avatar" aria-label={name || "Your profile"}>{name.trim().slice(0, 1).toUpperCase() || "R"}</span></div></header>
